...
def draw_lidar_frame(full_scan_map, bg_image, scale_mm_per_pixel=3, center=(960, 540)):
    ...
    min_dist = float('inf')
    max_dist = 0
    # Tứ giác ABCD (mm, degree) và contour pixel để kiểm tra điểm nằm trong tứ giác
    quad_points = [
        (943, 302),   # A
        (1750, 329),  # B
        (1750, 31),   # C
    ...
    quad_contour = np.array(quad_pixels, dtype=np.int32)
    # Vẽ các điểm LIDAR
    points_in_quad = []  # lưu các điểm (mm và pixel) nằm trong tứ giác để phân cụm
    for value in full_scan_map.values():
        ...
        if 0 <= x_px < 1920 and 0 <= y_px < 1080:
            # Kiểm tra nếu điểm nằm trong tứ giác ABCD -> vẽ đỏ, ngược lại vẽ xanh
            try:
                inside = cv2.pointPolygonTest(quad_contour, (x_px, y_px), False) >= 0
            except Exception:
                inside = False
            ...
            points_drawn += 1
            if inside:
                # Lưu thông tin để phân cụm (dùng tọa độ mm để tính khoảng cách 80mm)
                points_in_quad.append({
                    'x_mm': distance_mm * math.sin(math.radians(angle_deg)),
                    'y_mm': distance_mm * math.cos(math.radians(angle_deg)),
                    'x_px': x_px,
                    'y_px': y_px
    ...
    # Vẽ tứ giác ABCD (sử dụng quad_pixels đã tính phía trên)
    for i in range(4):
        cv2.line(display_image, tuple(quad_pixels[i]), tuple(quad_pixels[(i + 1) % 4]), (255, 0, 0), 2)
    labels = ['A', 'B', 'C', 'D']
    ...
    # Phân cụm các điểm nằm trong tứ giác ABCD sao cho trong một cụm
    # khoảng cách giữa hai điểm bất kỳ < 160mm.
    clusters = []
    if points_in_quad:
        n = len(points_in_quad)
        unassigned = set(range(n))
        # Greedy clique-building: tạo cluster mới, thêm điểm nếu nó cách mọi
        # điểm hiện có trong cụm <= 160mm (đảm bảo max pairwise < 160)
        while unassigned:
            i = unassigned.pop()
            cluster = [i]
            changed = True
            # Keep trying to add any remaining point that fits the clique
            while changed:
                changed = False
                to_check = list(unassigned)
                for j in to_check:
                    fits = True
                    jx = points_in_quad[j]['x_mm']
                    jy = points_in_quad[j]['y_mm']
                    for m in cluster:
                        mx = points_in_quad[m]['x_mm']
                        my = points_in_quad[m]['y_mm']
                        dx = jx - mx
                        dy = jy - my
    ...
    # Vẽ vòng tròn bao quanh mỗi cụm (dùng pixel coords, sẽ bị warp nếu transform sau đó)
    cluster_color = (255, 255, 0)  # vàng nhạt
    for ci, cluster in enumerate(clusters):
        if not cluster:
            continue
        pts_px = np.array([[points_in_quad[idx]['x_px'], points_in_quad[idx]['y_px']] for idx in cluster], dtype=np.float32)
        if pts_px.shape[0] == 1:
            cx, cy = pts_px[0]
            radius = 8
        else:
    ...
    # Thực hiện phép biến đổi phối cảnh để kéo tứ giác ABCD thành toàn màn hình
    try:
        src_pts = np.array(quad_pixels, dtype=np.float32)
        dst_pts = np.array([
            [0.0, 0.0],
            [0.0, 1080.0],
            [1920.0, 1080.0],
        ...
        if src_pts.shape == (4, 2):
            ...
            warped = cv2.warpPerspective(display_image, M, (1920, 1080))
            return warped
    except Exception as e:
        # Nếu có lỗi trong transform thì trả về ảnh gốc
