...
def get_perspective_transform_matrix(w:int = 1920, h:int = 1080):
    ...
    H=(500, 0)     # H (distance_mm, angle_deg)
    K=(1500, 0)    # K (distance_mm, angle_deg)
    ...
    TopMid=[w//2 - 1,0] # Toạ độ pixel của điểm H
    BottomMid=[w//2 - 1,h-1] # Toạ độ pixel của điểm K
    left_quad_polar_points = [A, B, K, H]
    right_quad_polar_points = [H, K, C, D]
    quad_polar_points = [A, B, C, D]
    left_quad_cartesian_points = [polar_to_cartesian(distance_mm, angle_deg) for distance_mm, angle_deg in left_quad_polar_points]
    right_quad_cartesian_points = [polar_to_cartesian(distance_mm, angle_deg) for distance_mm, angle_deg in right_quad_polar_points]
    quad_cartesian_points = [polar_to_cartesian(distance_mm, angle_deg) for distance_mm, angle_deg in quad_polar_points]
    left_quad_pixel_points = [TopLeft, BottomLeft, BottomMid, TopMid]
    right_quad_pixel_points = [TopMid, BottomMid, BottomRight, TopRight]
    quad_pixel_points = [TopLeft, BottomLeft, BottomRight, TopRight]
    left_source_points = np.array(left_quad_cartesian_points, dtype=np.float32)
    right_source_points = np.array(right_quad_cartesian_points, dtype=np.float32)
    source_points = np.array(quad_cartesian_points, dtype=np.float32)
    left_destination_points = np.array(left_quad_pixel_points, dtype=np.float32)
    right_destination_points = np.array(right_quad_pixel_points, dtype=np.float32)
    destination_points = np.array(quad_pixel_points, dtype=np.float32)
    M = cv2.getPerspectiveTransform(left_source_points, left_destination_points)
    W = cv2.getPerspectiveTransform(right_source_points, right_destination_points)
    MW = cv2.getPerspectiveTransform(source_points, destination_points)
    return M, W, MW
    
def detect_objects_in_lidar_data(full_scan_map, theta_scan):
    detected_polar_points = []
...
def cluster_detected_points(detected_pixel_points, distance_threshold=100):
...
if __name__ == "__main__":
    ...
    M, W, MW = get_perspective_transform_matrix()
    theta_scan = 80 
    # test example
    M_polar = (1400, 340) 
    M_cartesian = np.array([[polar_to_cartesian(M_polar[0], M_polar[1])]], dtype=np.float32)  # shape (1,1,2)
    M_pixel = cv2.perspectiveTransform(M_cartesian, M)[0][0]
    MW_pixel = cv2.perspectiveTransform(M_cartesian, MW)[0][0]
    print(polar_to_cartesian(M_polar[0], M_polar[1]))
    print("M_cartesian:", M_cartesian)
    print("M_pixel:", M_pixel)
    print("MW_pixel:", MW_pixel)
